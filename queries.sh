...
echo "$($PSQL "select name from teams full join games on teams.team_id = games.winner_id where year = 2014 and round = 'Eighth-Final' union all select name from teams full join games on teams.team_id = games.opponent_id where year = 2014 and round = 'Eighth-Final' order by name")"
